...
export function ApplicationForm({ jobId, jobTitle, companyName }: ApplicationFormProps) {
  ...
  if (status === 'success') {
    return (
      <div className="border border-[#CFC8BC] p-6 text-center">
        <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3">
          <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <p className="font-mono text-sm font-bold text-stone-900 mb-1">Application sent!</p>
  ...
  return (
    <div className="border border-[#CFC8BC] p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="applicantName" className="block font-mono text-xs tracking-widest uppercase text-stone-400 mb-1.5">
            Full Name
          </label>
          <input
            id="applicantName"
        ...
        <div>
          <label htmlFor="applicantEmail" className="block font-mono text-xs tracking-widest uppercase text-stone-400 mb-1.5">
            Email
          </label>
          <input
            id="applicantEmail"
